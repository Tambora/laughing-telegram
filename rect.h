...
class xz_rect: public hitable  {
  public:
    ...
    virtual float  pdf_value(const vec3 &o, const vec3 &v) const { // FIXME: Lights only possibly with xz rect, fix
        hit_record rec;
    ...
    virtual vec3 random(const vec3 &o) const {
        vec3 random_point = vec3(x0 + random0to1() * (x1 - x0), k,  z0 + random0to1() * (z1 - z0));
